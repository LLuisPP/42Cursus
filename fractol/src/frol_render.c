...
/*   Updated: 2024/02/12 10:11:43 by lprieto-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render(t_f *f, int x, int y)
{
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			re = f->prm.min_re + (double)x * (f->prm.max_re - (f->prm.min_re)) / WIDTH;
			im = f->prm.max_im + (double)y * (f->prm.min_im - f->prm.max_im) / HEIGHT;
            if (f->name != 0)
                // i = julia(re, im, f);
